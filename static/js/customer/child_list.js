  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      jQuery.each(data.data, function (i, item) {
        ...
        html = 
        `<div class="child_row" style="display:flex; color:black;">
        <div class="col-sm-2">
          ${item.first_name}
        </div>
        <div class="col-sm-2">
        ...
        <div class="col-sm-2">
          <div style="display:flex;"> 
            <div >
              <em for="dob"
              class="col-sm-4 col-form-label fas fa-edit"
              style="font-size: 18px; padding-right: 0px !important; "></em>
            </div>
            <div>
              <button type="submit" onclick="edit_child(${item.id})"
              id="child_form_${item.id}" style="border: none;background-color: white;">
              Edit
              </button>
          
            </div>
            <div>
            <button  onclick="delete_child(${item.id})"
            id="delete_${item.id}" style="border: none;background-color: white;"><i class='fa fa-trash' aria-hidden='true'></i>
            Delete
            </button>
            </div>
...
function edit_child(id){
    $("#child_form").show();
    $(".profile-header-official").hide();
    $(".profile-header-official").hide();
    // alert(id);
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#dob').val(data[0].birthdate);
            $('#medical_issue').val(data[0].medical_issue);
            localStorage.setItem("child_id", data[0].id);

            html = `PROFILE-CHILD`
            // html = `PROFILE-CHILD ${id}`

            $(".profile-header").append(html)
...
function allowEdit(){
    $("#first_name,#last_name,#dob,#medical_issue").attr("readonly", false);
    $('#first_name').focus();
}
