var tableLoad = $(document).ready(function() {
    $('#superuser-datatable').removeAttr('width').DataTable({
        dom: 'frtlip',
        "processing": true,
        "serverSide": true,
        "ajax": {
...
//                    M.toast({ html: JSON.parse(data.responseText).message, classes: 'red rounded' })
        ...
        "columns": [{
            ...
            { "data": "email" },
            {
                "data": "id",
                "render": function(data) {
                    var all_perms = '<button class="edit_btn" id=' + data + ' onclick=editSuperUser('+ data +') style="width:70px;"><i class="fa fa-edit">Edit</i></button> <button href="#deleteSuperUserModal" onclick="setSuperUser('+ data +')" class="delete" data-id="'+ data +'" data-toggle="modal" style="width:70px;"><i class="fa fa-trash" aria-hidden="true"></i> Delete</button>'
                    var edit_view = '<button class="edit_btn" id=' + data + ' onclick=editStaff('+ data +')><i class="material-icons prefix" style="width:70px;">Edit</i></button> <button class="view_btn" id=' + data + ' onclick=deleteStaff(id)><i class="material-icons prefix" style="width:70px;">visibility</i></button>'
...
//                    }
    ...
    var table = $("#superuser-datatable").dataTable().api();
