...
//    }
    $('#coach-datatable').removeAttr('width').DataTable({
        dom: 'frtlip',
        "processing": true,
        "serverSide": true,
        "ajax": {
...
//                    M.toast({ html: JSON.parse(data.responseText).message, classes: 'red rounded' })
        ...
        "columns": [{
            ...
            { "data": "address" },
            {
                "data": "id",
                "render": function(data) {
                    var all_perms = '<button class="edit_btn" id=' + data + ' onclick=editCoach('+ data +') style="width:70px;"><i class="fa fa-edit">Edit</i></button> <button href="#deleteEmployeeModal" onclick="setCoach('+ data +')" class="delete" data-id="'+ data +'" data-toggle="modal" style="width:70px;"><i class="fa fa-trash" aria-hidden="true"></i> Delete</button>'
...
//                    }
    ...
    var table = $("#coach-datatable").dataTable().api();
